...
Phase 2 user-confirmed assistant 复用同一 pure evaluator，并通过 STRM config IPC 暴露单样本 preview。renderer 只把 `MATCHED/HIGH` 且无 duplicate/conflict 的 canonical suggestion 转成普通 `USER` rule draft；不调用 `applyDiscovery()`，不写磁盘，也不 hot reload。离页不再隐式保存，只有现有显式 Save 才调用 `store.save()`，之后仍要求重启。preview/accepted diagnostics 只记录 status/confidence/count/reason 白名单，不记录路径。该 UI 层不进入 `libmpv.playInternal()`、`selectRule()`、`resolveAsync()` 或 CD2 service。
