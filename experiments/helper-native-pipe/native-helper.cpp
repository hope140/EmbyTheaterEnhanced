// Research-only native helper integration spike; GPL-2.0-only.
...
class JsonParser {
    ...
    Json value() {
        ...
        if (source[position] == '{') return objectValue();
        if (source[position] == '[') return arrayValue();
    ...
    Json arrayValue() {
        ...
        while (true) { result.array.push_back(value()); whitespace(); if (consume(']')) return result; if (!consume(',')) throw std::runtime_error("expected-comma"); }
    }
    Json objectValue() {
...
#define MPV_FUNCTIONS(X) \
    ...
    X(mpv_wait_event) X(mpv_terminate_destroy) X(mpv_command_async) X(mpv_get_property) \
    X(mpv_get_property_string) X(mpv_free) X(mpv_client_api_version) X(mpv_error_string) X(mpv_event_name)
...
#undef FIELD
    ...
    void observeGeneration(uint64_t generation) {
        ...
        struct Observation { const char* name; mpv_format format; } observations[] = {
            {"time-pos", MPV_FORMAT_DOUBLE}, {"duration", MPV_FORMAT_DOUBLE}, {"pause", MPV_FORMAT_FLAG},
            {"core-idle", MPV_FORMAT_FLAG}, {"path", MPV_FORMAT_STRING}
        };
        for (const auto& observation : observations) {
            uint64_t token = nextReplyUserdata++;
            if (p_mpv_observe_property(mpv, token, observation.name, observation.format) < 0) throw std::runtime_error("observe-property-failed");
            observerTokens[token] = {generation, observation.name}; activeObserverTokens.push_back(token);
        }
    }
    void mapMedia(int64_t mediaIdentity, uint64_t generation) {
        ...
        if (pending != quarantined.end()) { for (auto& record : pending->second) emitAttributed(record, generation); quarantined.erase(pending); }
    }
    ...
    void submitNextLoad() {
        ...
        const char* args[] = {"loadfile", command.path.c_str(), "replace", nullptr};
        int rc = p_mpv_command_async(mpv, command.replyUserdata, args);
    ...
    void handleEvent(mpv_event* event) {
        ...
        if (event->event_id == MPV_EVENT_END_FILE) {
            ...
            if (id >= 0) {
                openMedia.erase(id); quarantined.erase(id);
        ...
        if (event->event_id == MPV_EVENT_PROPERTY_CHANGE) {
            ...
            if (property->data && property->format == MPV_FORMAT_FLAG) value = *static_cast<int*>(property->data) ? "true" : "false";
...
public:
    Helper(std::string id, const std::string& dllPath, WriterQueue& output) : helperId(std::move(id)), writer(output) {
        std::wstring wide(dllPath.begin(), dllPath.end());
        library = LoadLibraryExW(wide.c_str(), nullptr, LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_DEFAULT_DIRS);
...
#undef LOAD
        ...
        option("config", "no"); option("terminal", "no"); option("idle", "yes"); option("input-default-bindings", "no");
        option("input-vo-keyboard", "no"); option("osc", "no"); option("vo", "null"); option("ao", "null");
        option("audio-display", "no"); option("keep-open", "no");
        ...
        emitGlobal("ready", "{\"protocolVersion\":1,\"mpvClientApiVersion\":" + std::to_string(api) +
            ",\"queueLimits\":" + writer.metricsJson() + '}');
    }
    ~Helper() { if (mpv) p_mpv_terminate_destroy(mpv); if (library) FreeLibrary(library); }
    void process(const Json& message) {
        ...
        if (method == "load") {
            ...
            std::string path = params.text("path"); if (path.empty() || path.size() > 4096) throw std::runtime_error("invalid-path");
            loadQueue.push_back({generation, request, nextReplyUserdata++, std::move(path)}); return;
        }
        ...
        if (method == "stop") {
            ...
            uint64_t userdata = nextReplyUserdata++; const char* args[] = {"stop", nullptr};
            int rc = p_mpv_command_async(mpv, userdata, args); if (rc < 0) { emitError(generation, request, "stop-queue-failed"); return; }
        ...
        if (method == "get-property") {
            ...
            std::string name = message.at("params").text("name"); char* value = p_mpv_get_property_string(mpv, name.c_str());
            std::string result = value ? quote(value) : "null"; if (value) p_mpv_free(value);
            emitResponse(generation, request, "{\"value\":" + result + '}' ); return;
        }
...
int main(int argc, char** argv) {
    ...
    try {
        InputReader reader(input); Helper helper(argv[2], argv[1], writer);
        while (true) {
            InboundMessage message;
            while (reader.pop(message)) {
                try { helper.process(message.json); }
                catch (const std::exception& error) {
                    writer.enqueue("{\"protocolVersion\":1,\"type\":\"lifecycle\",\"scope\":\"helper\",\"helperInstanceId\":" + quote(argv[2]) +
